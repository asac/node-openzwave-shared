	...
	"targets": [{
		...
		"conditions": [
			...
			["OS=='linux'", {
				...
				"cflags": [ "-Wno-ignored-qualifiers -Wno-write-strings -Wno-unknown-pragmas" , '<!@(<(pkg-config) --cflags libopenzwave)', ],
				'ldflags': [
                     		   '<!@(<(pkg-config) --libs-only-L --libs-only-other libopenzwave)',
		                ],
				'libraries': [
                        		'<!@(<(pkg-config) --libs-only-l libopenzwave)',
		                ],
"include_dirs": [
"<!(node -p -e \"require('path').dirname(require.resolve('nan'))\")",
